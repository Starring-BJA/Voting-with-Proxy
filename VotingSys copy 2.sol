...
contract VotingSystem3 is Initializable, OwnableUpgradeable {
    ...
    function executeProposal(uint proposalID) external onlyOwner{
        Proposal storage proposal = proposals[proposalID];
        require(proposal.endTime <= block.timestamp, "Voting has not finished yet");

        uint resultIndex = 0;
        uint resultVoteMax = proposal.votes[0];
        for(uint i=0; i<proposal.votes.length; i++){
            if(resultVoteMax < proposal.votes[i]){
                resultVoteMax = proposal.votes[i];
                resultIndex = i;
            }
        }
        proposal.result = resultIndex;
    }

    function getResultOfProposal(uint proposalID) external view returns(string memory result){
        Proposal storage proposal = proposals[proposalID];
        result = proposal.options[proposal.result];
    }
}